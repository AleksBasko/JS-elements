...
$(document).ready(function() {
    $('.menu-bar').on('click', function(){
        if ( $(this).hasClass('menu-bar__open')) {
            $(this).removeClass('menu-bar__open');
        }
        else {
            $(this).addClass('menu-bar__open');
        }
    });
});
$(document).ready(function() {
    ...
    $.getJSON('../../lessons/lesson_fruit.json', function (data) {
        var lesson_arr = data.arr;

        var numberItems = lesson_arr.length;
        var randMain = Math.floor(Math.random() * numberItems);
        var valueRandMain = lesson_arr[randMain].english;
        var idMainWord = $('.cards_rus--question');
        idMainWord.html(lesson_arr[randMain].english);

    });
});
$(document).ready(function () {
    $.ajax({
        url: '../../../lessons/lesson_fruit.json',
        dataType: 'json',
        type: 'get',
        cache: 'false',
        success: function (data) {
            var data_arr = data.arr;
            var lesson_arr = data_arr.slice();
            var second_arr = data_arr.slice();

            function learning_word() {
                var numberItems = lesson_arr.length;
                if (numberItems > 0) {
                    var randMain = Math.floor(Math.random() * numberItems);
                    var randAnswerPosition = Math.floor(Math.random() * 6);
                    var englishWord = lesson_arr[randMain].english;
                    var russianWord = lesson_arr[randMain].russian;
                    var pictureWord = "../../../img/" + lesson_arr[randMain].picture;
                    var questionWord = $('.select_question h2');
                    var answerTrueWord = $('.select_answer-item').eq(randAnswerPosition);
                    // var pictureWordBlock = $('.select_answer-item').eq();
                    questionWord.html(englishWord);
                    answerTrueWord.html(russianWord);
                    // pictureWordBlock.html("<img src=" + pictureWord + ">");

                    // var blockArr = [1, 2, 3, 4, 5, 6];
                    // blockArr.splice(randAnswerPosition - 1, 1);


                    var indexAnswer = $.inArray(lesson_arr[randMain], second_arr);
                    second_arr.splice(indexAnswer, 1);


                    var blocks = $('.select_answer-item');
                    for (var i = 0; i < blocks.length; i++) {
                        if (i == randAnswerPosition) {
                            continue;
                        }
                        var indexData = Math.floor(Math.random() * second_arr.length);
                        blocks.eq(i).html(second_arr[indexData].russian);
                        second_arr.splice(indexData, 1);
                        console.log(second_arr[indexData]);
                    }


                    lesson_arr.splice(randMain, 1);
                    second_arr = data_arr.slice();
                }
                else {
                    $('.cards-learn__picture').html("Вы выучили все слова в уроке");
                }
            }
...
//     while (data.length) {
//         for (var i = 0; i < blocks.length; i++) {
//             var indexData = Math.floor(Math.random() * data.length);
//             blocks.eq(i).html(data[indexData]);
        ...
        success: function (data) {
            ...
            function learning_word() {
                ...
                if (numberItems > 0) {
                    ...
                    var questionWord = $('.write-ask__item h2');

                    questionWord.html(russianWord);
                    ...
                    $('.write-ask__item h2').html("Вы выучили все слова в уроке");
                }
                function writer() {
...
//     var findArr = [];
//
//     var findPairItem = $('.find-pair__item');
//
//     var secondItem = '';
//
//     findPairItem.on('click', function(){
//
//
//         $(this).addClass('find-pair--selected');
//         firstItem.text = $(this).find('p').text();
//         firstItem.number = findPairItem.index($(this));
//
//         findArr[0] = firstItem;
//         console.log(findArr);
//         if( findArr.length == 2 ) {
//             if(findArr[0].text == findArr[1].text) {
//                 findPairItem.eq(findArr[0].number).addClass('find-pair--selected_true');
//                 findPairItem.eq(findArr[1].number).addClass('find-pair--selected_true');
//             }
//             findArr.splice(0, 2);
//         }
//     });
...
function findPair() {
    ...
    findPairItem.on('click', function(){
        ...
        if( findArr.length < 1 ) {
            firstItem.text = $(this).find('p').text();
            firstItem.number = findPairItem.index($(this));
            findArr.push(firstItem);
        }
        else {
            secondItem.text = $(this).find('p').text();
            secondItem.number = findPairItem.index($(this));
            findArr.push(secondItem);
            if(findArr[0].text == findArr[1].text) {
                findPairItem.eq(findArr[0].number).addClass('find-pair--selected_true');
                findPairItem.eq(findArr[1].number).addClass('find-pair--selected_true');
            }
...
$(document).ready(function() {
    var text_split = $('.split_mit').text();
    var arr_split = [];
    console.log(text_split[0]);
    for(var i=0;i<text_split.length;i++){
        if(i%3 == 0) {
            arr_split.push('1');
        }
        else {
            arr_split.push(text_split[i]);
        }
    }
        ...
        success: function (data) {
            ...
            function learning_word() {
                ...
                if (numberItems > 0) {
                    ...
                    var russianWord = lesson_arr[randMain].russian;
                    var pictureWord = "../../../img/" + lesson_arr[randMain].picture;
                    var firstWord = $('.cards-learn__question');
                    var secondWord = $('.cards-learn__answer');
                    var pictureWordBlock = $('.cards-learn__picture');
                    firstWord.html(englishWord);
                    secondWord.html(russianWord);
                    pictureWordBlock.html("<img src=" + pictureWord + ">");
                    lesson_arr.splice(randMain, 1);
                }
                else {
